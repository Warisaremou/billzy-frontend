...
export const textareaVariants = cva(
  "border-input placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 dark:bg-input/30 flex [field-sizing:content] min-h-16 w-full rounded-none border bg-transparent px-3 py-2 text-base shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
  {
    variants: {
      variant: {
        default: "",
        unstyled: "border-transparent h-8 shadow-none focus-visible:ring-0 focus-visible:border-input px-2",
      },
      error: {
...
export class HlmTextarea implements BrnFormFieldControl, DoCheck {
  ...
  protected readonly _computedClass = computed(() =>
    hlm(
      textareaVariants({ error: this._state().error, variant: this._state().variant }),
      this.userClass(),
  ...
  public readonly variant = input<TextareaVariants["variant"]>("default");

  protected readonly _state = linkedSignal(() => ({
    error: this.error(),
  ...
  public setError(error: TextareaVariants["error"]): void {
    this._state.set({ error, variant: this._state().variant });
  }
