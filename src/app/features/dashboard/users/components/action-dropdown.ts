    ...
    <hlm-alert-dialog>
      ...
      <hlm-alert-dialog-content *brnAlertDialogContent="let ctx">
        <hlm-alert-dialog-header>
          <h3 hlmAlertDialogTitle>Vous êtes sûr de vouloir supprimer l'utilisateur?</h3>
          <p hlmAlertDialogDescription>
            Cette action ne peut pas être annulée. Cette action supprimera définitivement le compte de l'utilisateur.
          </p>
        </hlm-alert-dialog-header>
...
export class UserActionDropdown {
  ...
  mutation = injectMutation(() => ({
    mutationFn: () => this.userService.deleteUser$(this.selectedUserId!),
    onSuccess: async () => {
    ...
    onError: (error: { error: { message: string[] | string } }) => {
      ...
      toast.error(errorMessage || "Une erreur est survenue lors de la suppression de l'utilisateur.");
    },
  }));
  ...
  openEditSheet() {
    this.router.navigate([], {
      ...
      queryParams: { action: "edit", userId: this.selectedUserId },
